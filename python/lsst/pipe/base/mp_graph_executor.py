...
class MPGraphExecutor(QuantumGraphExecutor):
    ...
    def _make_xgraph(
        ...
        if self._execution_graph_fixup:
            try:
                self._execution_graph_fixup.fixup_graph(xgraph, new_graph.quanta)
        ...
        if old_graph is not None:
            for qnode in old_graph:
                node_state = xgraph.nodes[qnode.nodeId]
                node_state["quantum"] = qnode.quantum
                node_state["task_node"] = new_graph.pipeline_graph.tasks[node_state["task_label"]]
        else:
            for quantum_id, quantum in new_graph.build_execution_quanta().items():
                node_state = xgraph.nodes[quantum_id]
                node_state["quantum"] = quantum
                node_state["task_node"] = new_graph.pipeline_graph.tasks[node_state["task_label"]]
        return xgraph
        ...
        while jobs.pending or jobs.running:
            ...
            for quantum_id in list(jobs.running):  # iterate over a copy so we can remove.
                ...
                for downstream_job in blocked:
                    ...
                    report.quantaReports.append(quantum_report)
                    jobs.setJobState(downstream_job, JobState.FAILED_DEP)
            ...
            while len(jobs.running) < self._num_proc and jobs.pending:
                job = jobs.jobs[jobs.pending.pop()]
