...
class PipelineTaskConnections(metaclass=PipelineTaskConnectionsMetaclass):
    ...
    * ``Output`` - A `lsst.daf.butler.DatasetType` that will be produced by an
    ...
    def getTemporalBoundsConnections(self) -> Iterable[str]:
        ...
        Notes
        -----
        The temproal bound is used to search for prerequisite inputs that are
        ...
        Tasks that do not have temporal dimensions that have calibration
        prerequisite inputs should always override this method, as the default
        is that quanta are not temporally bounded.
        """
        return ()
