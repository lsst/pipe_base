        ...
        for dimensions, (tasks_in_group, dataset_types_in_group) in query.grouped_by_dimensions.items():
            ...
            for task_node in tasks_in_group.values():
                ...
                for connection_name, finder in list(task_prerequisite_info.finders.items()):
                    ...
                    try:
                        query_results = list(
                            # We materialize here as a way to to a
                            # SELECT DISTINCT on the main query with a subset
                            # of its dimensions columns.  It'd be better to
                            # have a way to do this that just makes a subquery
                            # or a CTE rather than a temporary table.
...
class _AllDimensionsQuery:
    ...
    def log_failure(self, log: LsstLogAdapter) -> None:
        ...
        try:
            for message in self.butler_query.explain_no_results():
