...
class ArgumentParser(argparse.ArgumentParser):
    ...
    def __init__(self, name, usage = "%(prog)s input [options]", **kwargs):
        ...
        argparse.ArgumentParser.__init__(self,
            ...
            epilog = """Notes:
* --config, --configfile, --id, --loglevel, --trace and @file may appear multiple times;
...
""",
        ...
        **kwargs)
        ...
        self.add_argument("-L", "--loglevel", nargs="*", action=LogLevelAction,
            help="logging level for component", metavar="LEVEL|COMPONENT=LEVEL")
    ...
    def parse_args(self, config, args=None, log=None, override=None):
        ...
        namespace.log = log if log is not None else lsst.log.Log()
...
class LogLevelAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string):
        ...
        @param[in] values       a list of trace levels;
            ...
            where level is an integer or a keyword
        @param[in] option_string    option value specified by the user (a str)
        """
        for componentLevel in values:
            ...
            permittedLogLevels = set(('DEBUG', 'INFO', 'WARN', 'ERROR', 'FATAL'))
            if logLevelUpr in permittedLogLevels:
                logLevel = getattr(lsst.log, logLevelUpr)
            else:
                try:
                    logLevel = int(levelStr)
                except Exception:
                    self.error("loglevel=%r not int or one of %s" % (namespace.loglevel, permittedLogLevels))
            lsst.log.setLevel(component, logLevel)
