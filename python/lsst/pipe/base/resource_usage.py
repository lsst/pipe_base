...
class QuantumResourceUsage(pydantic.BaseModel):
    ...
    @staticmethod
    def make_arrow_fields() -> list[pyarrow.Field]:
        """Return a list of PyArrow field objects that could be used to put
        resource usage information into a table.
        """
        return [
            pyarrow.field("memory", pyarrow.float32()).with_metadata({"unit": "byte"}),
            pyarrow.field("start", pyarrow.date64()),
            pyarrow.field("prep_time", pyarrow.float32()).with_metadata({"unit": "s"}),
            pyarrow.field("init_time", pyarrow.float32()).with_metadata({"unit": "s"}),
            pyarrow.field("run_time", pyarrow.float32()).with_metadata({"unit": "s"}),
            pyarrow.field("run_time_cpu", pyarrow.float32()).with_metadata({"unit": "s"}),
        ]

    def make_arrow_row_dict(self) -> dict[str, object]:
        """Return a dictionary that can be passed as a row in
