...
class LabelSpecifier:
    ...
    iteration bounds, end bounds are *INCLUDED*.

    There are multiple definitions of range-based slicing for graphs that are
    not a simple linear sequence here.  The definition used here is the
    intersection of the tasks downstream of ``begin`` and the tasks upstream
    of ``end``, i.e. tasks no dependency relationship to a bounding task are
    not included.
    """
