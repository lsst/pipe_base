        ...
        *,
        ...
        with uri.open(mode="wb") as stream:
            with zipfile.ZipFile(stream, mode="w", compression=zipfile.ZIP_STORED) as zf:
                self = cls(zf, compressor, address_writer)
                self._write_single_model("header", header)
                if cdict is not None:
                    zf.writestr("compression_dict", cdict.as_bytes())
                self._write_single_model("pipeline_graph", SerializedPipelineGraph.serialize(pipeline_graph))
                yield self
                address_writer.write_to_zip(zf, address_filename, int_size=header.int_size)

    def _write_single_model(self, name: str, model: pydantic.BaseModel) -> None:
        ...
        self._write_single_block(name, json_data)

    def _write_single_block(self, name: str, json_data: bytes, no_compress: bool = False) -> None:
        """Write a single compressed JSON block as a 'file' in a zip archive.
        ...
        no_compress : `bytes`, optional
            If `True`, do not compress the data.
        """
        if not no_compress:
