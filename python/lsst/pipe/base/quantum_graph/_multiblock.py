...
class AddressRow:
    ...
    def read(cls, stream: IO[bytes], n_addresses: int, int_size: int) -> AddressRow:
        ...
        int_size : `int`
            Number of bytes to use for all integers.
        """
        key = uuid.UUID(bytes=stream.read(16))
...
class AddressWriter:
    ...
    def write(self, stream: IO[bytes], int_size: int) -> None:
        ...
        for key in sorted(self.indices.keys()):
            row = AddressRow(key, self.indices[key], [m[key] for m in self.addresses])
...
class AddressReader:
    ...
    def from_stream(cls, stream: IO[bytes], page_size: int, start_index: int = 0) -> AddressReader:
        ...
        rows_per_page = page_size // cls.compute_row_size(int_size, n_addresses)
    ...
    def find(self, key: uuid.UUID) -> AddressRow:
        ...
        guess_page_float = (guess_index_float - self.start_index) / self.n_rows
