...
class Storage(AbstractContextManager):
    """A helper class for the provenance scanner that manages access to its
    internal database.
    ...
    def finish_ingests(self, producer_ids: list[uuid.UUID]) -> None:
        ...
        with self.transaction() as connection:
            for id_batch in itertools.batched(producer_ids, self._config.query_batch_size):
                connection.execute(
                    sqlalchemy.delete(self.tables.to_ingest).where(
                        self.tables.to_ingest.c.producer_id.in_(id_batch)
                    )
                )
