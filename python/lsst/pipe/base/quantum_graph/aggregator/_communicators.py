...
class _Sentinal(enum.Enum):
    ...
    NO_MORE_INGEST_RETURNS = enum.auto()
    WRITE_REPORT = enum.auto()
    SUPERVISOR_DONE = enum.auto()
...
class SupervisorCommunicator:
    def __init__(
        ...
        self._to_supervisor: Queue[Report] = context.make_queue()
        self._to_scanners: dict[
            int, Queue[IngestConfirmation | Literal[_Sentinal.NO_MORE_INGEST_RETURNS]]
    ...
    def poll_resuming(self) -> Iterator[ScanReport]:
        n_done: int = 0
        while True:
            match self._handle_progress_reports(self._to_supervisor.get(block=True)):
                ...
                case _ResumeCompleted(n_quanta_loaded=n_quanta_loaded, n_ingests_loaded=n_ingests_loaded):
                    ...
                    n_done += 1
                    if n_done == len(self._to_scanners):
                        return
                case unexpected:
    ...
    def poll_scanning(self, timeout: float) -> Iterator[ScanReport]:
        block = True
        msg = _get_from_queue(self._to_supervisor, block=block, timeout=timeout)
    ...
    def shutdown(self) -> None:
        ...
        ingester_done = False
        n_scanners_done = 0
        while not (ingester_done and writer_done and n_scanners_done == len(self._to_scanners)):
            match self._handle_progress_reports(self._to_supervisor.get(block=True)):
                case None:
                    pass
                case _Sentinal.INGESTER_DONE:
                    ingester_done = True
                case _Sentinal.SUPERVISOR_DONE:
                    n_scanners_done += 1
        ...
        self._expect_empty_queue(self._to_supervisor)
...
class ScannerCommunicator:
    def __init__(self, supervisor: SupervisorCommunicator, scanner_id: int):
        ...
        self._to_supervisor = supervisor._to_supervisor
        self._to_scanner = supervisor._to_scanners[scanner_id]
    ...
    def poll_for_scan_requests(self) -> Iterator[tuple[uuid.UUID | None, IngestConfirmation | None]]:
        while True:
            if self._cancel_event.is_set():
                raise CancelError()
            ingest_return = _get_from_queue(self._to_scanner)
            assert ingest_return is not _Sentinal.NO_MORE_INGEST_RETURNS
            assert ingest_return is None or ingest_return.scanner_id == self.scanner_id
            ...
            if scan_request is _Sentinal.NO_MORE_SCAN_REQUESTS:
                if ingest_return is not None:
                    yield None, ingest_return
                return
            yield scan_request.quantum_id if scan_request is not None else None, ingest_return

    def poll_for_ingest_returns(self) -> Iterator[IngestConfirmation]:
        ...
        while True:
            if self._cancel_event.is_set():
                raise CancelError()
            ingest_return = self._to_scanner.get(block=True)
            if ingest_return is _Sentinal.NO_MORE_INGEST_RETURNS:
                self._to_supervisor.put(_Sentinal.SUPERVISOR_DONE)
                return
            assert ingest_return.scanner_id == self.scanner_id
            yield ingest_return
...
class IngesterCommunicator:
    def __init__(self, supervisor: SupervisorCommunicator):
        self._ingest_requests = supervisor._ingest_requests
        self._to_supervisor = supervisor._to_supervisor
        self._to_scanners = supervisor._to_scanners
        self._cancel_event = supervisor._cancel_event
    ...
    def poll(self) -> Iterator[IngestRequest]:
        ...
        while True:
            ...
            if ingest_request is _Sentinal.NO_MORE_INGEST_REQUESTS:
                n_requesters_done += 1
                if n_requesters_done == len(self._to_scanners):
                    return
                else:
                    continue
            yield ingest_request

    def send_no_more_ingest_returns(self) -> None:
        for q in self._to_scanners.values():
            q.put(_Sentinal.NO_MORE_INGEST_RETURNS)
...
class WriterCommunicator:
    def __init__(self, supervisor: SupervisorCommunicator):
        ...
        self._to_supervisor = supervisor._to_supervisor
        self._cancel_event = supervisor._cancel_event
        self._n_requesters = len(supervisor._to_scanners) + 1
    ...
    def poll(self) -> Iterator[ScanResult]:
        ...
        while True:
            ...
            if write_request is _Sentinal.NO_MORE_WRITE_REQUESTS:
                n_requesters_done += 1
                if n_requesters_done == self._n_requesters:
                    return
                else:
                    continue
            yield write_request
