...
class PredictedQuantumNodeDict(TypedDict):
    ...
    to be used explicitly, e.g.::
        ...
        node_data: PredictedQuantumNodeDict = xgraph.nodes[quantum_id]
    ...
    This attribute is only present if `build_execution_quanta` has been run
    on this node's quantum ID already.
    """


class PredictedDatasetNodeDict(TypedDict):
    """A typed dictionary that annotates the attributes of the NetworkX graph
    ...
    to be used explicitly, e.g.::
        ...
        node_data: PredictedDatasetNodeDict = xgraph.nodes[dataset_id]
...
class PredictedQuantumGraph:
    ...
    def _add_quantum(
        ...
        self._bipartite_xgraph.add_node(
            quantum_id, task_label=task_label, task_node=task_node, bipartite=NodeType.TASK.bipartite
        )
    ...
    def bipartite_xgraph(self) -> networkx.MultiDiGraph:
        ...
        and dataset nodes are only present for quanta whose ``quantum_datasets`
        were loaded.

        Node state dictionaries are described by the
        `PredictedQuantumNodeDict` and `PredictedDatasetNodeDict` type.
    ...
    def build_execution_quanta(
        ...
        for quantum_id in quantum_ids:
            quantum_node_dict: PredictedQuantumNodeDict = self._quantum_only_xgraph.nodes[quantum_id]
