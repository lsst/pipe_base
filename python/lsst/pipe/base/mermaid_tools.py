...
def _expand_dimensions(dimension_list: list[str], universe: DimensionUniverse) -> list[str]:
    """Return expanded list of dimensions, with special skypix treatment.

    Parameters
    ----------
    dimension_set : `list` [`str`]
...
def _render_task_node(
    task_id: str, taskDef: TaskDef, universe: DimensionUniverse, file: Any, show_dimensions: bool
) -> None:
    ...
    if show_dimensions and taskDef.connections and taskDef.connections.dimensions:
        ...
        if task_dims:
            ...
            lines.append(f"<i>dimensions:</i>&nbsp;{html.escape(dim_str)}")
...
def pipeline2mermaid(
    pipeline: Pipeline | Iterable[TaskDef], file: Any, show_dimensions: bool = True, show_storage: bool = True
) -> None:
    ...
    print(
        "classDef task fill:#B1F2EF,color:#000,stroke:#000,stroke-width:3px,font-family:Monospace,font-size:14px,text-align:left;",
        file=file,
    )
    print(
        "classDef ds fill:#F5F5F5,color:#000,stroke:#00BABC,stroke-width:3px,font-family:Monospace,font-size:14px,text-align:left,rx:10,ry:10;",
        file=file,
    )
    ...
    for idx, taskDef in enumerate(pipeline_tasks):
        ...
        _render_task_node(task_id, taskDef, universe, file, show_dimensions)
        ...
        for attr in sorted(iterConnections(taskDef.connections, "inputs"), key=lambda x: x.name):
            ...
            if attr.name not in allDatasets:
                _render_dataset_node(ds_id, attr.name, attr, universe, file, show_dimensions, show_storage)
            ...
            if component is not None and (nodeName, attr.name) not in allDatasets:
                ...
                if nodeName not in allDatasets:
                    _render_dataset_node(
                        ds_id_parent, nodeName, attr, universe, file, show_dimensions, show_storage
                    )
