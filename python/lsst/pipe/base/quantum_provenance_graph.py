...
from astropy.table.pprint import conf as pprint_conf
...
class Summary(pydantic.BaseModel):
    ...
    def pprint(self, brief: bool = False, datasets: bool = True, show_exception_diagnostics: bool = False,
               butler: Butler | None = None) -> None:
    ...
    def make_exception_diagnostics_table(
        self, butler=None, add_exception_msg=True, max_message_width=None, shorten_type_name=False
    ) -> astropy.table.Table:
        ...
        table : `astropy.table.Table`
            ...
            messages.
        """
        data_id_old = {'instrument': 'HSC', 'detector': 42, 'exposure': 318}
        import pickle
        with open('/home/en7908/notebooks/data.pkl', 'rb') as f:
            data = pickle.load(f)
        self.tasks = data
        ...
        # lookup later. Querying per data ID in the loop would be much slower.
        if butler:
            exposure_record_lookup = {d.dataId["exposure"]: d for d in butler.query_dimension_records("exposure", explain=False)}
        else:
            exposure_record_lookup = {}
            add_exposure_records = False

        if butler and not exposure_record_lookup:
            _LOG.warning("No exposure records found in the butler.")
            add_exposure_records = False
        ...
        for task_label, task_summary in self.tasks.items():
            exception_column_label = f"Exception in '{task_label}'"
            for type_name, exceptions in task_summary.exceptions.items():
                for exception in exceptions:
                    ...
                    assert rows[key][exception_column_label] == "", f"Duplicate entry for data ID {key} in {task_label}"
                    if shorten_type_name:
                        # Trim off the package name from the exception type for
                        # brevity.
                        type_name = type_name.rsplit(".", maxsplit=1)[-1]
                    rows[key][exception_column_label] = type_name
                    ...
                    if add_exposure_records:
                        exposure_record = exposure_record_lookup[data_id_old["exposure"]]
        ...
        return astropy.table.Table(table_rows)  #.group_by('detector')
