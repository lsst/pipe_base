...
class RemoveUnanchoredQuantaTestCase(unittest.TestCase):
    ...
    def test_remove_unanchored(self):
        ...
        n_source, n_downstream = self.skeleton.remove_unanchored_quanta("source", "anchor")
        self.assertEqual(n_source, 2)
        self.assertEqual(n_downstream, 0)
        ...
        n_source, n_downstream = self.skeleton.remove_unanchored_quanta("source", "anchor")
        self.assertEqual(n_source, 0)
        self.assertEqual(n_downstream, 0)

    def test_task_dropped_when_all_unanchored(self):
        """Task is dropped when all its quanta are removed."""
        n_source, _ = self.skeleton.remove_unanchored_quanta("source", "nonexistent")
        self.assertEqual(n_source, 4)
