...
from lsst.pipe.base.quantum_graph.aggregator import AggregatorConfig, Supervisor
...
class AggregatorTestCase(unittest.TestCase):
    ...
    @staticmethod
    @contextmanager
    def make_test_repo() -> Iterator[tuple[Butler, AggregatorConfig, PredictedQuantumGraph]]:
        ...
        config : `lsst.pipe.base.quantum_graph.aggregator.AggregatorConfig`
            Configuration for the scanner, initialized with paths in the
            temporary directory managed by the context manager.
        ...
        with patch_deterministic_uuid4(100):
            with DirectButlerRepo.make_temporary("base.yaml", "spatial.yaml") as (helper, root):
                ...
                pqgc.write(config.predicted_path)
                yield helper.butler, config, pqgc.assemble()
    ...
    def test_all_successful(self) -> None:
        ...
        with self.make_test_repo() as (butler, config, qg):
            ...
            self.assertCountEqual(executed_quanta, qg.quantum_only_xgraph.nodes.keys())
            Supervisor.run(config)
            with ProvenanceQuantumGraphReader.open(config.output_path) as reader:
                self.check_provenance_graph(qg, reader, butler, expect_failure=False)
    ...
    def test_some_failed(self) -> None:
        ...
        with self.make_test_repo() as (butler, config, qg):
            ...
            Supervisor.run(config)
            with ProvenanceQuantumGraphReader.open(config.output_path) as reader:
                self.check_provenance_graph(qg, reader, butler, expect_failure=True)
    ...
    def test_some_failed_two_phase(self) -> None:
        ...
        with self.make_test_repo() as (butler, config, qg):
            ...
            for _ in self.iter_graph_execution(config.butler_path, qg, raise_on_partial_outputs=True):
                pass
            config.assume_complete = False
            config.default_times["retry_timeout"] = 2.0
            config.default_times["wait"] = 1.0
