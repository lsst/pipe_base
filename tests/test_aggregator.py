...
class CompressionDictTestCase(unittest.TestCase):
    ...
    ``cannot train dict: Src size is incorrect`` otherwise.  We set the
    threshold at 10 to be safe.
    """
    ...
    def test_fallback_to_no_dictionary_with_too_few_samples(self) -> None:
        # 3 scans -> 9 samples, fewer than the 10 required by our code.
        writer = self.make_writer(3)
        cdict = writer.make_compression_dictionary()
        self.assertEqual(cdict.as_bytes(), b"")
        self.assertTrue(
            any("Only 9 < 10 samples" in message for message in writer.comms.log.messages),
